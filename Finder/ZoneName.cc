...
//      ps_elog(PSL_DEBUG, "Zone: %s\n", getName().c_str());
    }
    else
    {
      ps_elog(PSL_CRITICAL, "Could not generate zone!\n");
    }
  }
  // return current zone
  return names_; 
...
//      ps_elog(PSL_DEBUG, "New zone: %s\n", oss.str().c_str()  );
    ...
    {
      ps_elog(PSL_CRITICAL,"Format of X.in-addr.arpa is INCORRECT!\n");
    }
  }
